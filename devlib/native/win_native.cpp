...
namespace winutil {
    ...
    static auto initializeRootBuses(void) -> std::map<QString, int>
    {
        auto rootHubsBusesMap = std::map<QString, int>{};
        ...
        while (SetupDiEnumDeviceInfo(deviceInfoSet, _index, &devInfoData)) {
            _index++;
            ...
            rootHubsBusesMap[QString(instanceId)] = busNumber++;
        }
    ...
    static auto getUsbDevicesByContainerIdsMap(void) -> QMap<QString, DeviceProperties> {
    ...
    devlib::native::requestUsbDeviceList(void)
{
    ...
    auto usbDevicesByContainerIdsMap = winutil::getUsbDevicesByContainerIdsMap();
    ...
    winutil::foreachDevices(TEXT("USBSTOR"),
        [&devicesList, &usbDevicesByContainerIdsMap]
            ...
            if (usbDevProperties.instanceId.isEmpty()) {
                qCritical() << "Unable to find USB device using containerId";
                return;
            }
